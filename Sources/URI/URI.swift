...
@frozen public struct URI: Sendable {
    public var path: Path
    @usableFromInline var parameters: [Query.Parameter]?

    @inlinable public init(path: Path, query: Query? = nil) {
        self.path = path
...
extension URI {
    ...
    @inlinable public var query: Query? { self.parameters.map(Query.init(_:)) }
...
extension URI: ExpressibleByArrayLiteral {
    @inlinable public init(arrayLiteral: Path.Component...) {
        self.init(path: .init(components: arrayLiteral))
    }
}
extension URI: LosslessStringConvertible {
    @inlinable public init?(_ description: borrowing String) {
        self.init(description[...])
    }
    public init?(_ description: borrowing Substring) {
        do {
