extension URI {
    @frozen public struct Query: Sendable {
        public typealias Parameter = (key: String, value: String)

        public var parameters: [Parameter]

        @inlinable init(_ parameters: [Parameter]) {
            self.parameters = parameters
        }
    }
}
extension URI.Query: ExpressibleByDictionaryLiteral {
    @inlinable public init(dictionaryLiteral: (String, String)...) {
        self.init(dictionaryLiteral)
    }
}
extension URI.Query {
    static func += (string: inout String, self: Self) {
        var first: Bool = true
...
extension URI.Query: Equatable {
    public static func == (lhs: Self, rhs: Self) -> Bool {
        ...
        for (key, value): (String, String) in lhs.parameters {
            guard case nil = unmatched.updateValue(value, forKey: key) else {
                return false
            }
        }
        for (key, value): (String, String) in rhs.parameters {
            guard case value? = unmatched.removeValue(forKey: key) else {
                return false
            }
        }
        return true
