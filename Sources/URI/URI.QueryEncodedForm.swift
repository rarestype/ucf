extension URI {
    @frozen public struct QueryEncodedForm: Sendable {
        public var parameters: [Query.Parameter]

        private init(_ parameters: [Query.Parameter]) {
            self.parameters = parameters
        }
    }
}
extension URI.QueryEncodedForm {
    @inlinable public var query: URI.Query { .init(self.parameters) }
}
extension URI.QueryEncodedForm {
    ...
    private static func parse(utf8: consuming [UInt8]) throws -> Self {
    ...
    /// question mark (`?`).
    public static func parse(parameters: ArraySlice<UInt8>) throws -> Self {
