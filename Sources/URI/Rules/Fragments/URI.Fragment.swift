extension URI {
    @frozen public struct Fragment: Equatable, Hashable, Sendable {
        public var decoded: String

        @inlinable public init(decoded: String) {
...
extension URI.Fragment: RawRepresentable {
    ...
    @inlinable public init(rawValue: String) { self.init(decoded: rawValue) }

    /// Returns the percent-``decoded`` string.
    @inlinable public var rawValue: String { self.decoded }
}
...
extension URI.Fragment {
    ...
    /// hashtag (`#`).
    public init?(decoding string: Substring) {
        guard case (let decoded, _)? = try? PercentEncodedStringRule<
            URI.FragmentRule<String.Index>.UnencodedByte
        >.parse(string.utf8) else {
            return nil
        }
    ...
    @inlinable public var encoded: String { EncodingSet.encode(self.rawValue) }
}
extension URI.Fragment: CustomStringConvertible {
    /// Formats the fragment as a string with a leading hashtag (`#`).
    ///
    /// This property percent-encodes the fragment as needed.
    @inlinable public var description: String { "#\(self.encoded)" }
}
